...
def generar_fruta():
    # Busca posiciones libres (sin pared)
    posiciones_libres = []
    for y in range(len(nivel1)):
        for x in range(len(nivel1[y])):
            if nivel1[y][x] == ".":
                pos_x = -288 + (x * 48)
                pos_y = 288 - (y * 48)
                posiciones_libres.append((pos_x, pos_y))
    if posiciones_libres:
        x, y = random.choice(posiciones_libres)
        fruta = Fruta(x, y)
        frutas.append(fruta)

# --- Función de colisión con pared ---
def colision_con_pared():
    for pared in paredes:
        if jugador.distance(pared) < 24:
            print("¡Perdiste!")
            reiniciar_juego()
            return

# --- Función de colisión con fruta ---
...
def bucle_juego():
    ...
    # Genera fruta aleatoriamente cada cierto tiempo
    if random.randint(1, 20) == 1 and len(frutas) < 3:
        generar_fruta()
